...
### Lidstone model training ###
    ...
    output[12] = lidstone_model.calculate_prob(input_word)
    output[13] = lidstone_model.calculate_prob()
    output[14] = lidstone_model.calculate_prob_lambda(word=input_word)
    output[15] = lidstone_model.calculate_prob_lambda()
    lambda_var = [0.01, 0.1, 1.0]
    output[16] = lidstone_model.calculate_perplexity(lambda_var[0])
    output[17] = lidstone_model.calculate_perplexity(lambda_var[1])
    output[18] = lidstone_model.calculate_perplexity(lambda_var[2])
    output[20] = min(output[16:19])
    output[19] = lambda_var[output[16:19].index(output[20])]
    # output[21] =
